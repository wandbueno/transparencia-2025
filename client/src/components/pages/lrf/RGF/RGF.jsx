...
import { 
  ...
  getRelatorioRGF,
  getAnosDisponiveis,
  getUltimoSemestrePublicado,
  getUltimoAnoPublicado,
  getSemestresDisponiveis
} from "../../../../services/lrf/rgf";
...
const RGF = () => {
  ...
  useEffect(() => {
    document.title = `Relatório RGF - Portal Transparência - ${config.geral.nomeOrgao}`;
  ...
  const carregarDadosIniciais = async () => {
    try {
      ...
      const ultimoSemestre = await getUltimoSemestrePublicado(anoDisponivel);
      
      setParametros(prev => ({
        ...prev,
        ano: anoDisponivel,
        semestre: ultimoSemestre
      }));
  ...
  const carregarMetaDados = async () => {
    try {
      setLoading(true);
      const dados = await getMetaDadosRGF();
  ...
  const baixarRelatorio = async (tipo, titulo, path) => {
    ...
    try {
      const response = await getRelatorioRGF(tipo, parametros);
  ...
  return (
    <div className="container">
      ...
      <FilterSectionLRF 
        ...
        useSemestre={true}
        semestresDisponiveis={getSemestresDisponiveis()}
      />
