...
export const getMetaDadosMetas = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/meta-dados-relatorios`)
    return response.data
  } catch (error) {
    console.error('Erro ao buscar metadados das Metas:', error)
    throw error
  }
}

export const getRelatorioMetas = async (tipo, params = {}) => {
  try {
...
export const getUltimoMesPublicado = async (ano, tipo = 3) => {
  try {
    ...
    for (let mes = mesInicial; mes >= 1; mes--) {
      try {
        const response = await axios.get(`${API_BASE_URL}/metas-anuais`, {
          params: {
            ano,
            mes,
            extensao: 'pdf',
