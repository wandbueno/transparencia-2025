...
const Desoneracao = () => {
  ...
  useEffect(() => {
    document.title = `Desoneração - ${config.geral.nomeOrgao}`;
    fetchData();
  }, []);

  const fetchData = async () => {
    try {
      const result = await getPublicacoesPorTipo('Desoneração');
      setData(result);
      setFilteredData(result);
    } catch (error) {
      console.error('Erro ao carregar Desoneração:', error);
  ...
  return (
    <div className="container">
      ...
      {loading ? (
        ...
        <div>Erro ao carregar Desoneração: {error}</div>
      ) : (
        <DataTableComponent
          title="Desoneração"
          columns={columnsDesoneracao}
          data={filteredData}
        />
      )}
    </div>
